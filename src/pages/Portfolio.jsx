...
export default function Portfolio() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const chatRes = await axios.post(`${API}/chat`, {
        message: `Portfolio analysis:\n${summary}\nAvg score: ${avg}/100\n\nGive portfolio summary, what to hold, watch, reduce, and one action. Be brief.`,
        analysis_data: {},
        history: []
      });
      setAdvice(chatRes.data.response);
    } catch (e) {}
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-txt-1">Portfolio Tracker</h1>
      ...
      <div className="bg-panel border border-line-soft rounded-xl p-4 mb-5">
          ...
          {stocks.map((s, i) => (
            <div key={i} className="flex flex-col gap-1.5">
              <input
                ...
                className="bg-panel-2 border border-line text-txt-1 font-mono text-xs rounded-lg px-2.5 py-2 outline-none focus:border-accent/60 placeholder-txt-3"
              />
              <input
                value={s.ticker}
        ...
        <div className="flex gap-3 items-center">
          <select
            ...
            className="bg-panel-2 border border-line text-txt-1 font-mono text-sm rounded-lg px-3 py-2 outline-none"
          >
          ...
          <button
            onClick={analyze}
            disabled={loading}
            className="flex-1 bg-accent hover:bg-blue-600 text-white font-semibold py-2.5 rounded-lg transition-colors disabled:opacity-50"
          >
        ...
        <div className="space-y-3.5">
          <div className="bg-panel border border-line-soft rounded-xl p-4">
            ...
            <table className="w-full">
              <thead>
                <tr>
                  {['Company', 'Ticker', 'Risk Score', 'Label', 'VaR 99%', 'Regime'].map(h => (
                    <th key={h} className="font-mono text-xs text-txt-3 uppercase tracking-widest pb-2 text-right first:text-left border-b border-line">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {results.map((r, i) => (
                  <tr key={i}>
                    <td className="py-2.5 text-sm text-txt-1 border-b border-line-soft">{r.company}</td>
                    <td className="py-2.5 font-mono text-xs text-txt-3 text-right border-b border-line-soft">{r.ticker}</td>
                    <td className="py-2.5 font-mono text-sm text-txt-1 text-right border-b border-line-soft font-semibold">{r.composite_score}</td>
                    <td className="py-2.5 text-right border-b border-line-soft">
                      <span className={`font-mono text-xs px-2 py-0.5 rounded ${chipColor(r.label)}`}>{r.label}</span>
                    </td>
                    <td className="py-2.5 font-mono text-xs text-down text-right border-b border-line-soft">-{r.var_99}%</td>
                    <td className={`py-2.5 font-mono text-xs text-right border-b border-line-soft ${r.regime === 'High Vol' ? 'text-amber' : 'text-up'}`}>{r.regime}</td>
                  </tr>
                ))}
              </tbody>
            </table>
